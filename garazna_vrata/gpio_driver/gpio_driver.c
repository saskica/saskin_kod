...
void automat (events sig) //funkcija automat sa pravilima promena stanja
{
	...
	switch (current_state) //proverava trenutno stanje
	{
		...
		case Closing:
			...
			/*if (sig == Event_timedout) //ovo doraditi
			{
...
int gpio_driver_init(void)
{
    ...
    kt = ktime_set(timer_sec, 0);
    gate_timer.function = &gate_timer_callback;
    hrtimer_start(&gate_timer, kt, HRTIMER_MODE_REL);
	*/
    /* Initialize GPIO pins. */
    SetInternalPullUpDown(GPIO_23, PULL_DOWN);
    SetGpioPinDirection(GPIO_23, GPIO_DIRECTION_IN);

	result = gpio_request_one(GPIO_23, GPIOF_IN, "irq_gpio23");
	if(result != 0)
    {
        printk("Error: GPIO request failed!\n");
        goto fail_irq;
    }
    irq_gpio23 = gpio_to_irq(GPIO_23);
	result = request_irq(irq_gpio23, h_irq_gpio23,
      IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, "irq_gpio23", (void *)(h_irq_gpio23));
	if(result != 0)
    {
...
static ssize_t gpio_driver_write(struct file *filp, const char *buf, size_t len, loff_t *f_pos)
{
	...
	switch(gpio_driver_buffer[0]) {
		case 'o':
			automat(Event_open);
			printk("Door is opening");
			break;
		case 's':
			automat(Event_stop);
			printk("Door is stopped");
			break;
		case 'c':
			automat(Event_close);
